...
const move = (base1, x, rotate) => keyframes`
  from {
    ${base1}: 0;
...
const diagonalMove = (base1, base2, x, rotate) => keyframes`
  from {
    ${base1}: 0;
    ${base2}: 0;
    ${rotate ? `transform: rotate(0deg)` : ''};
  ...
  to {
    ${base1}: calc(${x}% - 25px);
    ${base2}: calc(${x}% - 25px);
    ${rotate ? `transform: rotate(259deg)` : ''};
  }
`;
...
class Box extends React.Component {
  ...
  getAnimation(x, base1, base2, bezier, time, isRotate, isDiagonal) {
    if (x) {
      const animation = isDiagonal 
        ? css`${diagonalMove(base1, base2, x, isRotate)} ${time}s ${bezier} infinite alternate`
        : css`${move(base1, x, isRotate)} ${time}s ${bezier} infinite alternate`;
      return animation;
    } else {
      return false;
    }
  ...
  render() {
    const { type, x, base1, base2, bezier, time, step, isRotate, isDiagonal } = this.props;
    const clipPath = this.getClipPath(type);
    const cubicBezier = this.getCubicBezier(bezier);
    const animation = this.getAnimation(x, base1, base2, cubicBezier, time, isRotate, isDiagonal);
